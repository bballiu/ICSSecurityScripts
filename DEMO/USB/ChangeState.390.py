...
def getState(s, state=''):
    ...
    00 80	dd 51
    """
    fullresp = binascii.hexlify(send_and_recv(s, 1000, 'cc01000f40070000eafa'))
    resp = fullresp[-4:]
    if fullresp[16:18] == '04': sState = "Stopped"  ## Is 02 when running
    # print(str(fullresp[16:18]))
    if binascii.hexlify(send_and_recv(s, 1000, 'cc01000f400800002db0'))[-4:] == '5703': sState = "Stopped"
...
print('----------------')
code = binascii.hexlify(send_and_recv(s, 1000, '0101001a005e000000000003000c494245544830314e305f4d00'))[34:36]
# code = send_and_recv(s, 1000, '0101001a005e000000000003000c494245544830314e305f4d00').encode('hex')[34:36]
send_and_recv(s, 1000, '01050016005f000008ef00' + code.decode() + '00000022000402950000')
ret = send_and_recv(s, 1000, '0106000e00610000881100' + code.decode() + '0400')

print('PLC Type  = ' + ret[30:50].decode())
print('Firmware  = ' + ret[66:70].decode())
print('Build     = ' + ret[79:100].decode())

send_and_recv(s, 1000,
              '0105002e00630000000000' + code.decode() + '00000023001c02b0000c0000055b4433325d0b466c617368436865636b3101310000')
send_and_recv(s, 1000, '0106000e0065ffffff0f00' + code.decode() + '0400')
send_and_recv(s, 1000, '010500160067000008ef00' + code.decode() + '00000024000402950000')
send_and_recv(s, 1000, '0106000e0069ffffff0f00' + code.decode() + '0400')
send_and_recv(s, 1000, '0102000c006bffffff0f00' + code.decode())
